function [solution, info] = runopf_ge_multitime(mpc)
...
%% --------------------------- decision vars ------------------------------
% gas
pi_sq   = sdpvar(n_gb, nt);            % squared pressure (p^2)
q_src   = sdpvar(n_gs, nt);            % source injection
q_flow  = sdpvar(n_gl, nt);            % link flow
lc_gas  = sdpvar(n_gas_dem, nt);       % gas load curtailment at demand nodes

% electricity (DC)
va      = sdpvar(n_b, nt);             % voltage angles
p_gen   = sdpvar(n_g, nt);             % generator outputs
lc_elec = 0;                          % keep your original (scalar 0)

% pipe relaxation variable
delta   = sdpvar(n_pipe, nt);

% compressor
comp_ratio_sq = sdpvar(n_comp, nt);    % squared compression ratio
...
q_gas_fired = p_gas_fired / GCV.ng * 3600 * 24 * eta.GFU;
