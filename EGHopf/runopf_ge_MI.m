function [solution, information] = runopf_ge_MI(mpc)
mpc0 = mpc;
%% initialization
...
n_gpp = size(i_gpp,1);
n_miss_direction = size(find(mpc.Gline_extra.Predirection ==0),1); %认为非管道的元件都事先给定方向了
i_miss_direction = find(mpc.Gline_extra.Predirection ==0);
...
%
gasFlowCons = [
        prs_sq(FB) - prs_sq(TB) == (2*w - gf.^2) ./ C.^2;
        0 <= w <= gf.^2;
        w <= alpha .* gf_max.^2;
        w >= gf.^2 - (1-alpha) .* gf_max.^2;
    ];
%% solve
cons = [
    boxCons;
...
penalty = 1e6;
objfcn = objfcn_IEGSoperatingCost(Pg,Qgs,mpc) + 1e-2*sum(gf.^2) + penalty*sum(LCg); % closer to mid level
